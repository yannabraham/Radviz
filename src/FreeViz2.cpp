...
void computeForcesDiscrete2(TPoint *pts, const TPoint *ptse, const int *classes, const int *clustWeights,
                          ...
                          )
{
  ...
  const int *clustWeightsi, *clustWeightsi2;
  int sumClustWeights;
  const int *classese;
    ...
    if (attractG != 0.0) {
      ...
      if (law == 0) {
        ...
        for(ptsi = pts + *classesi, ptsie = pts + classesi[1], clustWeightsi = clustWeights + *classesi; ptsi != ptsie; ptsi++, clustWeightsi++) {
          ...
          sumy += ptsi->y * *clustWeightsi;
          sumClustWeights += *clustWeightsi;
        }

//        sumx /= sumClustWeights;
//        sumy /= sumClustWeights;

        for(ptsi = pts + *classesi, ptsie = pts + classesi[1], Fai = Fa + *classesi, clustWeightsi = clustWeights + *classesi; ptsi != ptsie; ptsi++, Fai++, clustWeightsi++) {
          Fai->x = (sumx - sumClustWeights * ptsi->x)* *clustWeightsi;
          Fai->y = (sumy - sumClustWeights * ptsi->y)* *clustWeightsi;
//          Rcout << "Attractive forces_x are: " <<  Fai->x << std::endl;
        }
      }
//      Rcout << "Some of the attractive forces are: " << Fa[1:10] << std::endl;

      else if(law == 1) {
        for(ptsi = pts + *classesi, ptsie = pts + classesi[1], Fai = Fa + *classesi, clustWeightsi = clustWeights + *classesi; ptsi != ptsie; ptsi++, Fai++, clustWeightsi++) {
          Fai->x = Fai-> y = 0.0;
          for(ptsi2 = pts + *classesi, Fai2 = Fa + *classesi, clustWeightsi2 = clustWeights + *classesi; ptsi2 != ptsi; ptsi2++, Fai2++, clustWeightsi2++) {
            const double dx = ptsi->x - ptsi2->x;
            const double dy = ptsi->y - ptsi2->y;

            double fct;
//            if (law == TPNN::InverseSquare)
              fct = - sqrt(sqr(dx) + sqr(dy)) * *clustWeightsi * *clustWeightsi2;
//            else {
...
//              break;
          ...
          fct = fct * *clustWeightsi * *clustWeightsi2;
...
void symmetricTransformation2(TPoint *anc, TPoint *ance, bool mirrorSymmetry)
{
...
arma::mat optimizeAnchors2(arma::mat scaledData, NumericVector Rclasses, arma::mat anchors, NumericVector RclustWeights, double attractG, double repelG, int law, int steps, bool normalizeExamples)
{
    ...
    int *clustWeightsi, *clustWeights;
...
//    arma::mat pts2;
    ...
    clustWeights = (int *)malloc(nExamples * sizeof(int));
    ...
    for(clustWeightsi = clustWeights, i = 0; i < nExamples; clustWeightsi++, i++) {
      *clustWeightsi = RclustWeights[i];
    }
...
//        case 0:
          ...
          computeForcesDiscrete2(pts, ptse, classes, clustWeights, law, sigma2, attractG, repelG, dynamicBalancing != 0, Fa, Fr, nClasses);
//          break;
//        case 1:
...
symmetricTransformation2(anc, ance, mirrorSymmetry != 0);
//

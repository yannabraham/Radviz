...
btcells.df <- untreated.df %>% 
  ...
  unite('cellID',one_of(c('Treament','Cells','cellID')),sep = '_',remove = FALSE)
```
...
We can now visualize the effects of Vanadate and Ionomycine stimulations compared to unstimulated, but the contributions of individual channels are unclear and the effects of BCR/FcR-XL on B cells are unclear.
...
treat.S <- do.optimFreeviz(as.matrix(btcells.df[,btcells.channels]),
...
```{r}
btcells.S <- do.optimGraphviz(as.matrix(btcells.df[,btcells.channels]),btcells.graph)
